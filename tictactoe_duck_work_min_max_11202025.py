...
def MAX(board,alpha,beta):
    ...
    max_list.append(max_utility)
    max_list.append(new_action)
    print(max_list)
    return max_utility
...
def MIN(board,alpha,beta):
    ...
    min_list = []
    min_list.append(min_utility)
    min_list.append(new_action)
    print(min_list)
    return min_utility
...
def minimax(board):
    ...
    if player(board) == X:
        ...
        for action in actions(board):
           ...
           # min_v = min(min_utility,max_score)
            #max_v = max(max_utility, min_score)
